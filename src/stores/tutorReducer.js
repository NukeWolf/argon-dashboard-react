...
export const fetchTutees = createAsyncThunk("tutors/fetchTutees", async () => {
  const response = await client.get(process.env.REACT_APP_API_URL + "/tutees/");
  return response.data;
});
export const fetchTutors = createAsyncThunk("tutors/fetchTutors", async () => {
  const response = await client.get(process.env.REACT_APP_API_URL + "/tutors/");
  return response.data;
});
export const fetchRequests = createAsyncThunk(
  "tutors/fetchRequests",
  async () => {
...
export const finalizeRequest = createAsyncThunk(
  ...
  async (initialRequest) => {
    // We send the initial data to the fake API server
    const response = await client.post(
      process.env.REACT_APP_API_URL + "/finalize_request/",
      initialRequest
    );
    // The response includes the complete post object, including unique ID
    return response.data;
...
export const addNewTutor = createAsyncThunk(
  ...
  async (initialRequest) => {
    // We send the initial data to the fake API server
    const response = await client.post(
      process.env.REACT_APP_API_URL + "/tutors/",
      initialRequest
    );
    // The response includes the complete post object, including unique ID
    return response.data;
...
export const patchRequest = createAsyncThunk(
  ...
  async (initialRequest, thunkAPI) => {
    ...
    const response = await client.patch(
      process.env.REACT_APP_API_URL + "/requests/" + initialRequest.id + "/",
      initialRequest
    );
    // The response includes the complete post object, including unique ID
...
export const sendEmailAcceptance = createAsyncThunk(
  ...
  async (initialRequest, thunkAPI) => {
    // We send the initial data to the fake API server
    const response = await client.post(
      process.env.REACT_APP_API_URL + "/accept_request/",
      initialRequest
    );
    // The response includes the complete post object, including unique ID
    thunkAPI.dispatch(fetchRequests());
...
export const sendDone = createAsyncThunk(
  ...
  async (initialRequest, thunkAPI) => {
    // We send the initial data to the fake API server
    const response = await client.post(
      process.env.REACT_APP_API_URL + "/send_done/",
      initialRequest
    );
    // The response includes the complete post object, including unique ID
    thunkAPI.dispatch(fetchRequests());
...
export const checkTutor = createAsyncThunk(
  ...
  async (initialRequest, thunkAPI) => {
    // We send the initial data to the fake API server
    const response = await client.post(
      process.env.REACT_APP_API_URL + "/check_tutor/",
      initialRequest,
      {
        headers: { Authorization: "Token " + initialRequest.token },
      }
    );
    // The response includes the complete post object, including unique ID
    return response.data;
...
const requestProcess = (state, requests) => {
  ...
  for (let i = 0; i < requests.length; i++) {
    ...
    const tutor = state.tutors.tutors.filter((tut) => {
      return tut.id === request.Tutor;
    })[0];
    const tutee = state.tutors.tutees.filter((tut) => {
      return tut.id === request.Tutee;
    })[0];
...
export const selectPendingRequests = (state) => {
  const requests = state.tutors.requests.filter(
    (rq) => rq.status === "pending"
  );
  return requestProcess(state, requests);
};
export const selectFinalizedRequests = (state) => {
  const requests = state.tutors.requests.filter(
    (rq) => rq.status === "finalized"
  );
  return requestProcess(state, requests);
};
...
export const currentTutee = (state) => {
  const tutee = state.tutors.tutees.filter((tut) => {
    return tut.id === state.tutors.currentTuteeID;
  })[0];
